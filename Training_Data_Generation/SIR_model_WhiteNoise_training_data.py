#Training data generation of SIR model with additive WhiteNoise
...
for j in range(numSims):                
    ...
    #if intial infected negative; set random initial between (0, 0.5)        
    if I0 < 0:
        I0 = random.uniform(0, 0.5)                                 
...
if not os.path.exists('../Training_Data/data_train_WhiteNoise/output_sims_batch{}'.format(count)):
    os.makedirs('../Training_Data/data_train_WhiteNoise/output_sims_batch{}'.format(count)) 

if not os.path.exists('../Training_Data/data_train_WhiteNoise/output_resids_batch{}'.format(count)):
    os.makedirs('../Training_Data/data_train_WhiteNoise/output_resids_batch{}'.format(count))
...
for i in np.arange(numSims)+1:
    ...
    filepath='../Training_Data/data_train_WhiteNoise/output_sims_batch{}/tseries{}.csv'.format(count,sim_index+i)
    tseries_rename.to_csv(filepath,
                     index=False)    
    df_resids = df_ews.sort_index().loc[i,'I'].reset_index()[['Time','residuals']]
    filepath='../Training_Data/data_train_WhiteNoise/output_resids_batch{}/resids{}.csv'.format(count, sim_index+i)
    df_resids.to_csv(filepath,
                     index=False)

if not os.path.exists('../Training_Data/data_train_WhiteNoise/output_labels'):
    os.makedirs('../Training_Data/data_train_WhiteNoise/output_labels')
    
#label_indv
filepath = '../Training_Data/data_train_WhiteNoise/output_labels/label_batch{}.csv'.format(count)
label.to_csv(filepath,
            header=False, index=False) 

#store bifurcation_point
pd.DataFrame(tbif).to_csv('../Training_Data/data_train_WhiteNoise/output_labels/bifurcation_points_batch{}.csv'.format(count), header=False, index=False)
