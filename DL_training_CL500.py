...
# Set size of training library (100K) and length of time series (500)
(lib_size, ts_len) = (100000, 500)                                             
...
# get zipfile of resids
#all resids for DL model training are stored in a single folder named output_resids.zip
print('Load in zip file containing training data')
zf = zipfile.ZipFile('data_train/output_resids.zip'.format(ts_len))
...
#Takes pre-transition 500 points from residuals of forced simulations and last 500 from null simulations.
#If transition point is before 500; values after the transition point are set to zeros.
for tsid in tsid_vals:
    ...
    if ((t_transition < 1500-1) and (t_transition > ts_len)):
        values_500 = values[int(t_transition - ts_len + 1) : int(t_transition + 1)]
    elif ((t_transition < 1500-1) and (t_transition < ts_len)):
          values_500 = values[0:500]
    else:
          values_500 = values[-500:]
    sequences.append(values_500)
    transition_point.append(t_transition)

sequences = np.array(sequences)  

print(transition_point)

# labels and groups are stored in labels.csv and groups.csv files 
# Make sure you have appropriate labels and groups
# Get target labels for each data sample
df_targets = pandas.read_csv('data_train/output_labels/labels.csv'.format(ts_len),
                          index_col='sequence_ID')

# train/validation/test split denotations
df_groups = pandas.read_csv('data_train/output_groups/groups.csv'.format(ts_len),
                            index_col='sequence_ID')
...
model.add(Dropout(dropout_percent))
model.add(Dense(2, activation='softmax',kernel_initializer = initializer_param))                    
...
f_results.write("Simulation %d macro f1: %f. macro avg precision %f.  macro avg recall %f. Kernel size %d.  Filters %d.  Batch size %d.  Epochs %d.  seq_len %d.  Set size %d.  Mem_cells %d. Mem_cells2 %d. dropout %f. CNN layers %d, LSTM layers %d, pool_size_param %d, learning_rate_param %f.  initializer %s \r\n" % (kk, f1_score(test_target, test_preds, average='macro'),precision_score(test_target, test_preds, average='macro'),recall_score(test_target, test_preds, average='macro'),kernel_size_param,filters_param,batch_param,epoch_param,ts_len,lib_size,mem_cells,mem_cells2,dropout_percent,CNN_layers,LSTM_layers,pool_size_param,learning_rate_param,initializer_param))
f_results.flush()

f_results2.write("%d, %f, %f, %f, %d, %d, %d, %d, %d, %d, %d, %d, %f, %d, %d, %d, %f, %s \r\n" % (kk, f1_score(test_target, test_preds, average='macro'),precision_score(test_target, test_preds, average='macro'),recall_score(test_target, test_preds, average='macro'),kernel_size_param,filters_param,batch_param,epoch_param,ts_len,lib_size,mem_cells,mem_cells2,dropout_percent,CNN_layers,LSTM_layers,pool_size_param,learning_rate_param,initializer_param))
f_results2.flush()
