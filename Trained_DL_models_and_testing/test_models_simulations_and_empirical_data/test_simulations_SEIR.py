#Test simulations of SEIR model with additive WhiteNoise
...
# Model
def de_fun_S(S,I,beta,d):
  return A-d*S-beta*S*I

def de_fun_E(S,E,I,beta,d,ka):
  return beta*S*I-(d+ka)*E

def de_fun_I(E,I,ka,d,gamma):
  return ka*E-(d+gamma)*I

def de_fun_R(I,R,gamma,d):
  return gamma*I-d*R
...
print('\nBegin simulations \n')
for j in range(numSims):                                                          #numSims=10
    # noise intensity
    sigma_S = np.random.triangular(0, 0.5, 1)                                    # noise intensity
    sigma_E = np.random.triangular(0, 0.5, 1)
    sigma_I = np.random.triangular(0, 0.5, 1)
    sigma_R = np.random.triangular(0, 0.5, 1)
    ...
    dW_E_burn = np.random.normal(loc=0, scale=sigma_E*np.sqrt(dt), size = int(tburn/dt))
    dW_E = np.random.normal(loc=0, scale=sigma_E*np.sqrt(dt), size = len(t))
    ...
    for i in range(int(tburn/dt)):                                               
        S0 = S0 + de_fun_S(S0,I0,beta[0],d)*dt + dW_S_burn[i]        
        if np.isnan(S0):
            S0 = random.uniform(0, 500)
        E0 = E0 + de_fun_E(S0,E0,I0,beta[0],d,ka)*dt + dW_E_burn[i]
        if np.isnan(E0):
            E0 = random.uniform(0, 1)
        I0 = I0 + de_fun_I(E0,I0,ka,d,gamma)*dt + dW_I_burn[i]
        if np.isnan(I0):
            I0 = random.uniform(0, 1)
        R0 = R0 + de_fun_R(I0,R0,gamma,d)*dt + dW_R_burn[i]
        if np.isnan(R0):
            R0 = random.uniform(0, 500)
    
    #if intial infected negative; set random initial between (0, 1)
    if I0 < 0 :
        I0 = random.uniform(0, 1)
    ...
    for i in range(len(t)-1):                                                      
        S[i+1] = S[i] + de_fun_S(S[i],I[i],beta.iloc[i],d)*dt + dW_S[i]           
        E[i+1] = E[i] + de_fun_E(S[i],E[i],I[i],beta.iloc[i],d,ka)*dt + dW_E[i]
        I[i+1] = I[i] + de_fun_I(E[i],I[i],ka,d,gamma)*dt + dW_I[i]
        R[i+1] = R[i] + de_fun_R(I[i],R[i],gamma,d)*dt + dW_R[i]
    ...
    data = {'tsid': (j+1)*np.ones(len(t)),                                      
                ...
                'I': I,
                'R': R}
    df_temp = pd.DataFrame(data)                                                 #data for each j 
    list_traj_append.append(df_temp)                                             #combined data for all j 
    ...
    noise_intensity_E.append(sigma_E)
    noise_intensity_I.append(sigma_I)
    noise_intensity_R.append(sigma_R)

label = pd.concat(label_list, ignore_index=True)
df_traj = pd.concat(list_traj_append)                                            #Concatenate(merge) DataFrame from each j 
